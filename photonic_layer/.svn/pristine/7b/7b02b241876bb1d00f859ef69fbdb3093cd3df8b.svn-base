package People.Meisam.GUI.Utilities.ImportData;
...
public class ImportDataController {
	...
	private void createTable(String filePath){
		...
		if(isNumeric(words[0]) || isNumeric(MoreMath.evaluate(words[0])+"")){
			numRows++ ;
...
//				values = MoreMath.Arrays.append(values, Double.parseDouble(data[i][ColumnNumber])) ;
				values = MoreMath.Arrays.append(values, MoreMath.evaluate(data[i][ColumnNumber])) ;
			}
		}
		return values ;
	}
