...
public class Benes8x8MZISwitch implements Experiment {
	...
	public void run(AbstractResultsManager man, AbstractResultsDisplayer dis)
		...
		int stateNumber = 0 ;
		for(int i=0; i<20; i++){
			if(state[i]=="B") {stateNumber += 0 * Math.pow(2, i) ;}
			else{stateNumber += 1 * Math.pow(2, i) ;}
		}
...
//		dp.addProperty("State of all Switches", stateSwitches);
		dp.addProperty("Switch Number", stateNumber);
...
/*		for(int i=0; i<8; i++){
			checkDataPoint(dp, stateSwitches, ports3[i].absSquared(), i, 3, -10, 0);
		}*/
		...
		for(int i=0; i<8; i++){
			checkDataPoint(dp, stateSwitches, ports9[i].absSquared(), i, 9, -20, 0);
		}
		
//		dp.addResultProperty("Port 1 Out", checkNaN(10*Math.log10(ports9[0].absSquared())));


//		checkDataPoint(dp, stateSwitches, ports9[1].absSquared(), 1, 9, -20, -5);
