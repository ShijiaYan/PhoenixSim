package People.Meisam.GUI.Utilities.ExportPlot.ExportToMATLAB.Extras.OldVersions;
...
public class ExportToMatlabController_old extends AbstractController {
...
//		markerColor = color_to_RGB255(marker_color) ;
	...
	private void initializeFontSize(ComboBox<String> fontBox){
		double[] fontSize = MoreMath.linspace(10, 60, 5d) ;
...
//	}
	...
	private String[] getGenericCode(){
		...
		String[] code = {} ;
		String st0 = "x =" + xVar.toString() + ";" ;
		String st1 = "y = " + yVar.toString() + ";" ;
		String st2 = "ph = plot(x, y) ;" ;
		String st3 = "ax = gca ;" ;
		code = MoreMath.Arrays.concat(code, new String[] {st0, st1, st2, st3}) ;
		return code ;
	}
	...
	private String[] getPlotStyleCode(){
		...
		code = MoreMath.Arrays.concat(code, new String[] {st0, st1, st2, st3, st4, st5, st6, st7}) ;
		return code ;
	}
	...
	private String[] getColorsCode(){
		...
		code = MoreMath.Arrays.concat(code, new String[] {st0, st1}) ;
		return code ;
	}
...
//			st0 = "ax.XLabel.String = " + "'" + xVar.getName() + "'" + ";" ;
		...
		if(xAxisAuto.isSelected()){
			...
			if(xAxisMin.getText().isEmpty()){
				s1 = MoreMath.Arrays.FindMinimum.getValue(xVar.getAllValuesRemoveNaNs()) + "" ;
			}
			else{
				s1 = xAxisMin.getText() ;
			}
			...
			if(xAxisMax.getText().isEmpty()){
				s2 = MoreMath.Arrays.FindMaximum.getValue(xVar.getAllValuesRemoveNaNs()) + "" ;
			}
			else{
				s2 = xAxisMax.getText() ;
			}
...
//			st0 = "ax.YLabel.String = " + "'" + yVar.getName() + "'" + ";" ;
		...
		if(yAxisAuto.isSelected()){
			...
			if(yAxisMin.getText().isEmpty()){
				s1 = MoreMath.Arrays.FindMinimum.getValue(yVar.getAllValuesRemoveNaNs()) + "" ;
			}
			else{
				s1 = yAxisMin.getText() ;
			}
			...
			if(yAxisMax.getText().isEmpty()){
				s2 = MoreMath.Arrays.FindMaximum.getValue(yVar.getAllValuesRemoveNaNs()) + "" ;
			}
			else{
				s2 = yAxisMax.getText() ;
			}
	...
	private String[] getLegendCode(){
		...
		if(legendDisabled.isSelected()){
			...
			code = MoreMath.Arrays.concat(code, new String[]{st0}) ;
		}
