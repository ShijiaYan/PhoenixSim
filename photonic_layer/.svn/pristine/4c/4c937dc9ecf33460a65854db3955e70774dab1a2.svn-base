...
Complex J = new Complex(0,1), one = new Complex(1,0), zero = new Complex(0,0) ;
	...
	public ModeSlabWgTM(
		...
		this.asymmetryFactor = slab.getTMsymmetryFactor() ;
	}
	
	// Finally need to solve the eigen value equation
	private double getModeEquation(double b, int modeNumber){
		double aTM = asymmetryFactor ;
		double factSubs = n_core * n_core / (n_down * n_down) ;
		double factClad = n_core * n_core / (n_up * n_up) ;
