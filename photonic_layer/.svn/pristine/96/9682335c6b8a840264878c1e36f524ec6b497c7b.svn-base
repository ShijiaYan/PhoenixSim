...
public class Benes8x8MZISwitchStageByStage implements Experiment {
	...
	public Benes8x8MZISwitchStageByStage(
	...
	public Complex[][] getAllOutputPortsAllStages(int inPortNumber){
		// Stage 0 ports
		Complex[] ports0 = new Complex[8] ;
		for(int i=0; i<8; i++){
			if(i==inPortNumber-1){ports0[i] = one ;}
		...
		Complex[][] ports_transposed = {ports0, ports1, ports2, ports3, ports4, ports5, ports6, ports7, ports8, ports9} ;
		Complex[][] ports = new Complex[8][10] ;
		for(int i=0; i<8; i++){
			for(int j=0; j<10; j++){
				ports[i][j] = ports_transposed[j][i] ;
			}
		}
		return ports ;
	}
	...
	public Complex[] getAllOutputPortsSpecificStage(int inPortNumber, int stageNumber){
		// Stage 0 ports
		Complex[] ports0 = new Complex[8] ;
		for(int i=0; i<8; i++){
			if(i==inPortNumber-1){ports0[i] = one ;}
		...
		return ports_transposed[stageNumber] ;
	}
	...
	public void run(AbstractResultsManager man, AbstractResultsDisplayer dis)
		...
		String fileName = "Benes8x8AllStages" ;
		// step2: create the file
		FileOutput fout = new FileOutput(fileName + ".txt", "a") ;
		for(int i=0; i<M; i++){
			// step3: defining the line
			String line0 =  getSwitchState() + "     " + "Port "+(i+1)+ " Excited" + "      " ;
			fout.println(line0); 
			String line1 = "----------------------------------------------------------------------" ;
			fout.println(line1);
			Complex[][] stages = getAllOutputPortsAllStages(i+1) ;
			for(int k=0; k<M; k++){
				String line2 = "" ;
				for(int j=0; j<N; j++){
					line2 += String.format("%4.4f", MoreMath.Conversions.todB(stages[k][j].absSquared())) + "     " ;
