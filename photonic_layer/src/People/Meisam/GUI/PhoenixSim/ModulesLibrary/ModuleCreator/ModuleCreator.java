package People.Meisam.GUI.PhoenixSim.ModulesLibrary.ModuleCreator;

import java.io.IOException;
import People.Meisam.GUI.Builders.WindowBuilder;
import People.Meisam.GUI.PhoenixSim.MainModule.PhoenixSimModuleController;
...
public class ModuleCreator {
	...
	public void createHpeModule(){
		// loading the GUI
		WindowBuilder builder = new WindowBuilder(loader) ;
		builder.setIcon("/People/Meisam/GUI/ThermalHeater/Extras/PhoenixSim_logo.png");
		try {
			builder.build_NoModality("HPE Project", false);
		} catch (IOException e) {
			e.printStackTrace();
		}
		this.controller = loader.getController()  ;
		...
		// adding necessary tabs
		controller.addWelcomeTabHPE();
//		controller.addHpeSymmetricRingTab();
//		controller.addHpeSymmetricRingRingTab();
		// select welcome tab
		controller.tabs.getSelectionModel().select(0);
		// set close policy for clearing JRE from memory
		if(simDataBase == null){
