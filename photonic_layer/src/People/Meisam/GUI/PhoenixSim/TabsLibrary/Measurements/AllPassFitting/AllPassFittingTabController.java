package People.Meisam.GUI.PhoenixSim.TabsLibrary.Measurements.AllPassFitting;
...
public class AllPassFittingTabController extends AbstractTabController {
	...
	RadioButton fitTodB;
	@FXML
	RadioButton fitToNondB;
	@FXML
	TextField fsrTextField;
	@FXML
	Label fsrLabel;
	@FXML
	TextField drLossTextField;
	@FXML
	Label drLossLabel;
	...
	public void chooseXData() throws IOException {
		...
		controller.getSetExitButton().setOnAction(e -> {
			xData = new SimulationVariable(controller.getVariable().getName(), controller.getVariable().getAlias(),
	...
	public void chooseYData() throws IOException {
		...
		controller.initialize();
		controller.getSetExitButton().setOnAction(e -> {
			yData = new SimulationVariable(controller.getVariable().getName(), controller.getVariable().getAlias(),
					controller.getValues());
			yDataLabel.setText("Y data is set to '" + yData.getName() + "'");
			controller.getSetExitButton().getScene().getWindow().hide();
			if (xData != null && yData != null) { fig = createPlot(xData, yData); showPlot(fig, matlabPane); }
	...
	public void calculateKappa() {
		...
		if (fitTodB.isSelected()) {
			...
			// step 1: find BW & ER
			double ER_dB = (interpolator.interpolate(lambda_min_nm) + y_dB_max) / 2
					- interpolator.interpolate(lambda_res_nm);
			...
			Function fitFunc = new Function() {
				...
				public double evaluate(double[] values, double[] parameters) {
					...
					double tr_dB = MoreMath.Conversions.todB(getLorentzian(BW, er, lambda_nm, lambda_res_nm)) + y_dB_min
							+ ER_dB;
					return tr_dB;
				}
			...
			if (L_plus < 1 && t_plus < 1) {
				t = t_plus;
				L = L_plus;
			} else if (L_minus < 1 && t_minus < 1) { t = t_minus; L = L_minus; }
