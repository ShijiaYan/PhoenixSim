package People.Meisam.GUI.PhoenixSim.TabsLibrary.Measurements.ElectroOpticFitting.PINdiode.PINac;
...
public class PINacFittingTabController extends AbstractTabController {
	...
	Pane matlabPane;
	@FXML
	TextField iDataTextField;
	@FXML
	Label iDataLabel;
	@FXML
	TextField vDataTextField;
	@FXML
	Label vDataLabel;
	@FXML
	...
	public void setIData() {
		...
		if (iData != null && vData != null) { fig = createPlot(iData, vData); showPlot(fig, matlabPane); }
	}
	...
	public void chooseIData() throws IOException {
		...
		controller.getSetExitButton().setOnAction(e -> {
			iData = new SimulationVariable(controller.getVariable().getName(), controller.getVariable().getAlias(),
	...
	public void chooseVData() throws IOException {
		...
		controller.initialize();
		controller.getSetExitButton().setOnAction(e -> {
			vData = new SimulationVariable(controller.getVariable().getName(), controller.getVariable().getAlias(),
					controller.getValues());
			vDataLabel.setText("ZL data is set to '" + vData.getName() + "'");
			controller.getSetExitButton().getScene().getWindow().hide();
			if (iData != null && vData != null) { fig = createPlot(iData, vData); showPlot(fig, matlabPane); }
	...
	public void calculate() {
		...
		double[] freq_values = MoreMath.linspace(MoreMath.Arrays.FindMinimum.getValue(freq_Ghz),
				MoreMath.Arrays.FindMaximum.getValue(freq_Ghz), 1000);
