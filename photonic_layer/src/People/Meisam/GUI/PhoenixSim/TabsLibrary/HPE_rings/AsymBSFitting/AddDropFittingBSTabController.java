package People.Meisam.GUI.PhoenixSim.TabsLibrary.HPE_rings.AsymBSFitting;
...
import hpe.edu.lrl.fitting.AddDropRingBSAsymFitting;
...
public class AddDropFittingBSTabController extends AbstractTabController {

	AddDropRingBSAsymFitting adr;
	...
	Pane matlabPane;
	@FXML
	TextField xDataTextField;
	@FXML
	Label xDataLabel;
	@FXML
	TextField yDataTextField;
	@FXML
	Label yDataLabel;
	@FXML
	TextField minLambdaTextField;
	@FXML
	Label minLambdaLabel;
	@FXML
	TextField maxLambdaTextField;
	@FXML
	Label maxLambdaLabel;
	@FXML
	TextField centerLambdaTextField;
	@FXML
	Label centerLambdaLabel;
	@FXML
	ListView<String> resultListView;
	@FXML
	RadioButton plusPhase;
	@FXML
	RadioButton minusPhase;
	@FXML
	TextField fsrTextField;
	@FXML
	Label fsrLabel;
	...
	public void setMinLambda() {
		...
		if (!st.isEmpty()) {
			try {
				...
				simDataBase.addNewVariable(
						new SimulationVariable("minLambda_(nm)", "min Lambda (nm)", new double[] { minLambda_nm }));
	...
	public void setMaxLambda() {
		...
		if (!st.isEmpty()) {
			try {
				...
				simDataBase.addNewVariable(
						new SimulationVariable("maxLambda_(nm)", "max Lambda (nm)", new double[] { maxLambda_nm }));
	...
	public void setCenterLambda() {
		...
		if (!st.isEmpty()) {
			try {
				...
				simDataBase.addNewVariable(new SimulationVariable("centerLambda_(nm)", "center Lambda (nm)",
						new double[] { centerLambda_nm }));
	...
	public void calculateKappa() {
		...
		while (M_start <= xData.getLength()) {
			if (xData.getValue(M_start) < lambda_min_nm) {
				M_start++;
			} else {
				break;
			}
		...
		while (M_end > 0) {
			if (xData.getValue(M_end) > lambda_max_nm) {
				M_end--;
			} else {
				break;
			}
		...
		Function fitFunc = new Function() {
			...
			public double evaluate(double[] values, double[] parameters) {
				...
				double phi = parameters[1]; // phase difference (degree)
				double r = parameters[2]; // lumped reflector field reflection
				double offset_dB = parameters[3];
				double Ls = parameters[4];
				adr = new AddDropRingBSAsymFitting(lambdaNm, lambda_res_nm, FSR_nm, kappa, kappa, Ls, r, phi);
				Complex S41;
		...
		if (plusPhase.isSelected()) {
			fit.setParameters(new double[] { 0.1, 50, 0.01, -30, 0.999 });
		} else {
			fit.setParameters(new double[] { 0.1, -50, 0.01, -30, 0.999 });
		}
		...
		for (int i = 0; i < fitted_plot.length; i++) {
			adr = new AddDropRingBSAsymFitting(lambda_nm[i], lambda_res_nm, FSR_nm, kappa, kappa, Ls, r1, phi);
			Complex S41;
			adr.sfg.buildForwardPaths("DC1.N1", "DC2.N4");
			S41 = adr.sfg.computeForwardGain().divides(adr.sfg.computeDelta());
			fitted_plot[i] = MoreMath.Conversions.todB(S41.absSquared()) + offset_dB;
		}
