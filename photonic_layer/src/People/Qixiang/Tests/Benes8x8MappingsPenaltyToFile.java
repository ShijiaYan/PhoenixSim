...
public class Benes8x8MappingsPenaltyToFile extends AbstractSimulation implements Experiment {
	...
	public Benes8x8MappingsPenaltyToFile(
	...
	public double getXtalkPenalty(int inPortNumber, int outPortNumber){
		...
		double xtalkArg = 0.5 * 6 * xtalkPower/signalPower ;
	...
	public void run(AbstractResultsManager man, AbstractResultsDisplayer dis)
		...
		if(port1dBm>-10 && port2dBm>-10 && port3dBm>-10 && port4dBm>-10 && port5dBm>-10 && port6dBm>-10 && port7dBm>-10 && port8dBm>-10){
			...
			fo.print("("+k+")  "+ getSwitchState());
			k++ ;
			fo.printtab();
			fo.print("1-->"+port1Map+": "+"lossPP(dB) = "+ -port1dBm +", XtalkPP(dB)= "+getXtalkPenalty(1, port1Map));
			fo.printtab();
			fo.print("2-->"+port2Map+": "+"lossPP(dB) = "+ -port2dBm +", XtalkPP(dB)= "+getXtalkPenalty(2, port2Map));
			fo.printtab();
			fo.print("3-->"+port3Map+": "+"lossPP(dB) = "+ -port3dBm +", XtalkPP(dB)= "+getXtalkPenalty(3, port3Map));
			fo.printtab();
			fo.print("4-->"+port4Map+": "+"lossPP(dB) = "+ -port4dBm +", XtalkPP(dB)= "+getXtalkPenalty(4, port4Map));
			fo.printtab();
			fo.print("5-->"+port5Map+": "+"lossPP(dB) = "+ -port5dBm +", XtalkPP(dB)= "+getXtalkPenalty(5, port5Map));
			fo.printtab();
			fo.print("6-->"+port6Map+": "+"lossPP(dB) = "+ -port6dBm +", XtalkPP(dB)= "+getXtalkPenalty(6, port6Map));
			fo.printtab();
			fo.print("7-->"+port7Map+": "+"lossPP(dB) = "+ -port7dBm +", XtalkPP(dB)= "+getXtalkPenalty(7, port7Map));
			fo.printtab();
			fo.print("8-->"+port8Map+": "+"lossPP(dB) = "+ -port8dBm +", XtalkPP(dB)= "+getXtalkPenalty(8, port8Map));
			fo.println();
