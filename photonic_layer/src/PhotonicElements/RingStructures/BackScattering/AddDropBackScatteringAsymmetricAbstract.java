...
public class AddDropBackScatteringAsymmetricAbstract {
	...
	public AddDropBackScatteringAsymmetricAbstract(
			...
			@ParamName(name="Lumped Reflector 1 Model") LumpedReflector LR1,
			@ParamName(name="Lumped Reflector 2 Model") LumpedReflector LR2
			){
		...
		this.FSR_nm = FSR_nm ;
		this.deltaPhiRad = 2*Math.PI*(inputLambda.getWavelengthNm()-resLambdaNm)/FSR_nm + LR1.S21.phase() + LR2.S21.phase()  ;
//		this.deltaPhiRad = 2*Math.PI*(inputLambda.getWavelengthNm()-resLambdaNm)/FSR_nm  ;
	...
	public Complex[] excitePorts(Complex port1In, Complex port2In, Complex port3In, Complex port4In){
		...
		for(int i=0; i<steps; i++){
			DC1.connectPorts(port1In, port2In, LR1.port1, LR2.port1);
			LR1.connectPorts(DC1.port3, Cwg1.port2);
			Cwg1.connectPorts(DC2.port3, LR1.port2);
			DC2.connectPorts(port4In, port3In, Cwg1.port1, Cwg2.port1);
			Cwg2.connectPorts(DC2.port4, LR2.port2);
			LR2.connectPorts(DC1.port4, Cwg2.port2);
