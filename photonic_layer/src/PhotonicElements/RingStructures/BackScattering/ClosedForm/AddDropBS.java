package PhotonicElements.RingStructures.BackScattering.ClosedForm;
...
public class AddDropBS {

	public Wavelength inputLambda ;
	public LumpedReflector LR ;
	public double resLambdaNm, FSR_nm, deltaPhiRad, k1, t1, k2, t2, r, t, L ;
	Complex O ;
	...
	public AddDropBS(
		...
		O = Complex.minusJ.times(deltaPhiRad).exp().times(t1*t2*Math.sqrt(L)) ;
	...
	private Complex calculateS31(){
		...
		Complex num = Complex.minusJ.times(deltaPhiRad/2).exp().times(a) ;
		return num.divides(denum) ;
	}
	
	private Complex calculateS41(){
		Complex denum = O.times(O).plus(one).minus(O.times(2*t)) ;
		Complex a = Complex.minusJ.times(deltaPhiRad/2).exp().times(k1*k2*Math.pow(L, 0.25)) ;
