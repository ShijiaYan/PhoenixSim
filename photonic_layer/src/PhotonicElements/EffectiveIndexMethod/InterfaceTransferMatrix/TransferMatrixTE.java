package PhotonicElements.EffectiveIndexMethod.InterfaceTransferMatrix;

import PhotonicElements.Utilities.MathLibraries.Complex;
import PhotonicElements.Utilities.MathLibraries.ComplexMatrix;
import PhotonicElements.Utilities.Wavelength;

public class TransferMatrixTE {
...
/*		Complex kNormalFirst_raw = thetaFirst.cos().times(kFirst) ;
		kNormalFirst = new Complex(kNormalFirst_raw.re(), -Math.abs(kNormalFirst_raw.im())) ;
		Complex kNormalSecond_raw = thetaSecond.cos().times(kSecond) ;
		kNormalSecond = new Complex(kNormalSecond_raw.re(), -Math.abs(kNormalSecond_raw.im())) ;*/
	}
	//****************************************************************************************************
	...
	private ComplexMatrix getMaterialMatrix(double eps, double mu, Complex kNormal){
		...
		elements[1][0] = one.divides(plusJ.times(omega*mu)).times(minusE).times(minusJ).times(kNormal) ;
		elements[1][1] = one.divides(plusJ.times(omega*mu)).times(plusE).times(plusJ).times(kNormal) ;
