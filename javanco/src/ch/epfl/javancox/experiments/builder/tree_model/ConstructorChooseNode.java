...
public class ConstructorChooseNode extends AbstractChooseNode {
    ...
    private transient final static ch.epfl.general_libraries.logging.Logger logger =
            new ch.epfl.general_libraries.logging.Logger(
                    ConstructorChooseNode.class);
    ...
    protected ConstructorChooseNode(Constructor<?> c, Class<?> cl, ObjectConstructionTreeModel containingTree,
                                    final String def, boolean checkDef) throws Exception {
        ...
        if (annotations.length != parameters.length) {
            throw new IllegalStateException(
                    "Parameter - annotations mismiatch. Likely means using a non-static internal class, which is " +
                            "impossble");
        }
        if (def != null && parameters.length == 1) {
            ...
            addParameterChooseNode(parameters[0], AbstractChooseNode.parseAnnotations(annotation),
                    getContainingTreeModel(), checkDef);
            ...
            for (Class<?> param : parameters) {
                boolean siblingStatus = addParameterChooseNode(param,
                        AbstractChooseNode.parseAnnotations(annotations[i]), getContainingTreeModel(), checkDef);
    ...
    private boolean addParameterChooseNode(Class<?> param, Map<String, String> parsedAnnot,
                                           ObjectConstructionTreeModel<?> tree, boolean checkDef) throws Exception {
        AbstractChooseNode an;
        if (param.isArray()) {
            an = new ArrayChooseNode(param, parsedAnnot, tree, checkDef);
        } else if (ClassUtils.isBooleanType(param)) {
            an = new BooleanChooseNode(Boolean.class, parsedAnnot, tree, checkDef);
        } else if (ClassUtils.isTypableType(param)) {
            an = new TypableChooseNode(param, parsedAnnot, tree, checkDef);
    ...
    @Override
    protected void restoreCreated() {
        if (this.loadFailed) {
            this.setConfigured(false);
            this.removeAllChildren();
            this.removeFromParent();
        } else if (this.configured) {
            this.createdValue = this.getContainingTreeModel().addConfiguredConstructor(this);
        }
    }
    ...
    private void writeObject(java.io.ObjectOutputStream out) throws IOException {
        ...
        for (ConstructorNodeChooserPointer pairs : pointed) {
    ...
    public Object clone() {
        try {
            ConstructorChooseNode ret = new ConstructorChooseNode(this.getConstructor(), this.constructedClass,
                    getContainingTreeModel(), null, false);
    ...
    class SpecialIterator implements Iterator<Pair<Object, ObjectRecipe>> {
            ...
            if (this.paramSize > 0) {
                ...
                for (Object param : children) {
                    if (((AbstractParameterChooseNode) param).configured) {
                        Iterator<Pair<Object, ObjectRecipe>> iteratorForParamI = ((AbstractChooseNode) param)
                                .iterator();
                        ...
                        if (iteratorForParamI.hasNext()) {
                            ...
                            throw new IllegalStateException(
                                    "Recently created iterator has no element to provide at all");
                        }
                        ++i;
                    }
                }
        ...
        Pair<Object, ObjectRecipe> findNext() {
            ...
            try {
                ...
                if (e.getCause() instanceof WrongExperimentException) {
                    ...
                    throw new IllegalStateException("Potentiall trying to run a constructor not public or erroneous",
                            e.getCause());
                }
            } catch (Exception e) {
        ...
        private void checkNext() {
            ...
            while (!this.hasNext && i < this.paramSize) {
                this.hasNext = this.iterators.get(i++).hasNext();
            }
        }
