...
import ch.epfl.general_libraries.charts.CustomBarRenderer;
import ch.epfl.general_libraries.charts.CustomCategoryDataset;
import ch.epfl.general_libraries.charts.CustomLogarithmicAxis;
import ch.epfl.general_libraries.charts.CustomNumberAxis;
import ch.epfl.general_libraries.charts.TextureSupplier;
...
public class BarChartProvider extends AbstractChartProvider {
	...
	public ChartContainer computeChart(DataRetrievalOptions options, AdvancedDataRetriever retriever) {
		...
		if (isScalar) {
			...
			return createBarChart(meth2, options, retriever);
		}
	}
	...
	public void createExcelData(AdvancedDataRetriever retriever) throws Exception {
		...
		String separator = ch.epfl.general_libraries.utils.ListSeparatorRetriever.retrieve();
	...
	private ChartContainer createScalarChart(String meth2, DataRetrievalOptions options, AdvancedDataRetriever retriever) {
		...
		jFreeChart.getCategoryPlot().setRenderer(renderer);
			...
			for (int i = 0; i < length; ++i) {
				...
				setSeriePropertyBar(renderer, noSerie, crit.get(0), crit.size() > 1 ? crit.get(1) : null, 0 /* colors not implemented here*/);
	...
	private ChartContainer createBarChart(String meth2, DataRetrievalOptions options, AdvancedDataRetriever retriever) {
		...
		int categories = dataseries.size();
		for (DataSeries cv : dataseries) {
			...
			if (row >= 0) {
				setSeriePropertyBar(renderer, dataset.getRowIndex(rowKey),
						cv.crit.get(0), cv.crit.size() > 1 ? cv.crit.get(1) : null, categories);
			}
		}
		...
		if (!multiplesValues.isEmpty()) {
			for (Entry<Pair<Comparable<?>, Comparable<?>>, Pair<Integer, List<Double>>> entry : multiplesValues
				...
				double[] all = StatFunctions.getAll(list,
	...
	private void setSeriePropertyBar(BarRenderer renderer, int noSerie,
			PairList<String,String> color, PairList<String,String> shape, int cateNb) {
		/*	if (color == null)
			color = "";
		if (shape == null)
			shape = "";
		...
		if (paint == null) {
			...
			Color c = (Color) paint;
			if (c.getRGB() == new Color(255, 255, 85).getRGB())
				paint = drawingSupplier.getNextPaint();
