...
public class StatisticalDistribution<K extends Comparable<K>> implements Iterable<Map.Entry<Object, Integer>> {
	...
	public static class DoubleDistribution extends StatisticalDistribution<Double> {
		...
		public double[] getSortedValues() {
			double[] toReturn = new double[this.samples];
			int position = 0;
			for (Map.Entry<Object, Integer> entry : this.struct.entrySet()) {
				Double val = (Double)entry.getKey();
				Integer instances = entry.getValue();
				for (int i = 0 ; i < instances ; i++) {
					toReturn[position] = val;
					position++;
				}
			}
			return toReturn;
	...
	protected TreeMap<Object, Integer> struct;
