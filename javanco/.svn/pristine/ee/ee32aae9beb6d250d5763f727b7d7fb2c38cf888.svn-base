...
public class BFS {
	...
	private static Path[] getShortestPaths(AbstractGraphHandler agh, int startNode, int mode, LayerContainer layer) {
		...
		int[] pred = getBFSTree(agh, startNode, mode, 0, layer);
	...
	private static int[] getBFSTree(AbstractGraphHandler agh, int node, int mode, int returnMode, LayerContainer layer) {
		...
		int[] visited = new int[agh.getHighestNodeIndex()+1];
	...
	public static int[] getTree(AbstractGraphHandler agh, int i) {
		return getBFSTree(agh, i, 6, 1, null);
	}
