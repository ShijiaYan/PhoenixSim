...
public class MoreMaths {
	...
	public static double product(double[] array, int fromIndex, int toIndexExcluded) {
		double h = 1;
		for (int i = fromIndex ; i < toIndexExcluded ; i++) {
			h *= array[i];
		}
	...
	public static int sum(int[] array, int fromIndex, int toIndexExcluded) {
		int h = 0;
		for (int i = fromIndex ; i < toIndexExcluded ; i++) {
			h += array[i];
		}
	...
	public static double evaluatePolynom(double[] factors, double x) {
		double sum = 0;
		for (int i = 0 ; i < factors.length ; i++) {
			sum += Math.pow(x, factors.length-1-i)*factors[i];
		}
		return sum;
	}
