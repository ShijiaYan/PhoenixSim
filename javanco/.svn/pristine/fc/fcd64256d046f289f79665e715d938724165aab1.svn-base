...
public class LayeredTreeGenerator extends AbstractDeterministicGenerator
	...
	/*	for (int i = 0 ; i <= layers ; i++) {
			int nodesOnThisLayer = (int)MoreMaths.product(siblings, i, layers);
			for (int j = 0 ; j < nodesOnThisLayer ; j++) {
				agh.newNode(0,i*100);
			}
		}
		int offset = 0;
		for (int i = 0 ; i < layers ; i++) {
			int nodesOnThisLayer = (int)MoreMaths.product(siblings, i, layers);
			for (int j = 0 ; j < nodesOnThisLayer ; j++) {
				int modulo = (int)MoreMaths.product(siblings,0, i+1);
				agh.newLink(offset + j, offset + ((offset + j) / modulo));
			}
			offset += nodesOnThisLayer;
		}*/		
		NodeContainer current = agh.newNode(0, 0);
		addLayer(agh, current, 0);
		int[] order = BFS.getTree(agh, 0);
		System.out.println(order);
		lay.assignNodesPosition(agh);
		for (int i = 0 ; i < order.length ; i++) {
			agh.changeNodeIndex(i, order.length+1+i);
		}
		for (int i = 0 ; i < order.length ; i++) {
			agh.changeNodeIndex(order.length+1+order[i], i);
		}
	}
