...
public class DetailedSourceDestAnalyser extends AbstractLWSimAnalyser {
	...
	public DetailedSourceDestAnalyser(@ParamName(name="Per source?") boolean src, 
									  @ParamName(name="Per source-dest?") boolean perSrcDest, 
									  @ParamName(name="Pcolor?") boolean withPColor) {
		this.perSrc = src;
			...
			for (int i = 0 ; i < clients ; i++) {
				...
				if (perSrcDest) {
					for (int j = 0 ; j < clients ; j++) {
						...
						perRoute.addResultProperty("per_route_receptions", emittedDropReceivedRetranPerSrcDest[i][j][2]);
						perRoute.addResultProperty("per_route_retransmissions", emittedDropReceivedRetranPerSrcDest[i][j][3]);				
	...
	public void packetRetransmitted(Message m) {
		if (perSrc)
			emittedDropReceivedRetranPerSrcDest[m.origin][m.dest][3]++;
