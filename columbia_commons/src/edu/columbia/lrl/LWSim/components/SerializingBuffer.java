...
public class SerializingBuffer extends AbstractTrafficOrigin implements TrafficTransitPoint {
	...
	public SerializingBuffer(int maxSize, double bufferLatency, int index, double outputRateCoeff,
	...
	public SerializingBuffer(int maxSize, double bufferLatency, int index) {
	...
	protected void processProducerEvent(Evt e) {
		size++;
		if (!transmitting) {
			double start = e.getTimeNS() + bufferLatency;
			transmit(start, bufferLatency + getEOTTimeNS(start, e.getMessage()), e.getMessage());
		} else {
			enqueue(e);
		}
	}
	...
	protected void processConsumerEvent(Evt e) {
		...
		if (nextInLine != null) {
			transmit(t, getEOTTimeNS(t, nextInLine.getMessage()), nextInLine.getMessage());
		} else {
			transmitting = false;
		}
	}
	...
	protected Evt pollEvt() {
		if (queues != null && queues.length >= 2 && queues[0] != null && queues[1] != null) {
			if (queues[0].size() > 0 && queues[1].size() > 1) { queues[1].peek(); queues[0].peek(); }
		}
		...
		for (int i = queues.length - 1; i >= 0; i--) {
			...
			if (q != null && q.size() > 0) { return q.poll(); }
		}
		return null;
	}
	...
	private PriorityQueue<Evt> getQueue(int priority) {
		if (queues == null) { queues = new PriorityQueue[priority + 1]; }
		if (queues.length <= priority) {
			PriorityQueue[] newA = new PriorityQueue[priority + 1];
			System.arraycopy(queues, 0, newA, 0, queues.length);
			queues = newA;
		}
		if (queues[priority] == null) queues[priority] = new PriorityQueue<Evt>();
		return (PriorityQueue<Evt>) queues[priority];
	}

	protected void transmit(double startTime, double endTime, Message msg) {
		if (nextDest == null)
			throw new IllegalStateException("Next destination of a buffer (id: " + this.index + ") not set");
		...
		scheduleEOTEvt(endTime, msg);
		transmitting = true;
	}

	protected void scheduleEOTEvt(double endTime, Message msg) {
		// Evt self = new Evt(getEOTTimeNS(startTimeNS, msg), this, this, 1, msg);
		Evt self = new Evt(endTime, this, this, 1, (Message) null);
	...
	public void notifyEnd(double ref, double status) {
		if (timeline != null) lwSimExperiment.addTimeLine(timeline);
	}
