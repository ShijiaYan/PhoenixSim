...
public class PowerConsumption {
	...
	private SimpleMap<String, Double> factorList = new SimpleMap<String, Double>();
	
	public PowerConsumption(String origin, boolean isDynamic, boolean isPerInterface, boolean isPerWavelength, double powerInMW) {
	...
	public void setWavelength(boolean b) {
		isPerWavelength = b;
	}	
	...
	public String toString() {
		return "Consumption of " + powerInMW + "mW (" +
				(isDynamic ? "dynamic" : "static") +
				(isPerInterface ? "per interface" : "global") +
				(isPerWavelength ? "per wavelength" : "per link (not per wavelength");
	}

	public static double compute(List<PowerConsumption> consumptions, double circuit, double utilization, int opticalInterfaces, int wavelengths) {
		double accum = 0;
	...
	public static void multiply(List<PowerConsumption> list,
			String factorName, int factor) {
