...
public class AvlTreeRm {
	 ...
	 * Removes a node from the tree, if it is existent.
	 */
	public int removeAndRank(int k) {
		// First we must find the node, after this we can delete it.
		return removeAVLAndRank(this.root, k);
	}
	 ...
	 *            The KEY of node to remove.
	 */
	public int removeAVLAndRank(AvlNode p, int q) {
		int dist = 0;
		if (p == null) {
			// do not find a node with key q
			return -1;
			...
			if (p.key > q) {
				...
				if (p.right != null)
					dist += p.right.weight;
				int next = removeAVLAndRank(p.left, q);
				if (next < 0)
					return -1;
				dist += next;
			} else if (p.key < q) {
				dist += removeAVLAndRank(p.right, q);
			} else if (p.key == q) {
				// we found the node in the tree.. now lets go on!
				if (p.right != null)
					dist += p.right.weight;
	...
	public void debug(AvlNode n) {
		...
		System.out.println("Left: " + l + " Key: " + n + " Right: " + r
				+ " Parent: " + p + " Balance: " + n.balance + " Weight: " + n.weight);
