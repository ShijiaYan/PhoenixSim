...
public class NACKFast_Deadline_NQPBuffer extends NACKFast_NQPrioritizedBuffer {
	...
	public NACKFast_Deadline_NQPBuffer(int maxSize, double bufferLatency, double spinetSwitchingTimeNS, int index,
			PriorityMode pm, boolean quenchEnabled, boolean deadlineBasedBackoff) {
		...
		if (this.quenchEnabled) {
			...
			for (int i = 0; i < this.nbClient; i++) {
				while (priorityQueue[i].size() > 0) {
					Message first = (Message) priorityQueue[i].firstEntry().getValue();
	...
	protected void pickAndConsume(Evt e) {
		...
		for (int i = 0; i < this.nbClient; i++) {
			...
			// find the EDF packet
			if (this.priorityQueue[i].size() > 0 && (Integer) this.priorityQueue[i].firstKey() < ED) {
				ED = (int) (Integer) this.priorityQueue[i].firstKey();
				EDIndex = i;
			}

			// find the FEDF packet
			if (this.priorityQueue[i].size() > 0 && (Integer) this.priorityQueue[i].firstKey() < feasibleED
		...
		if (true) throw new IllegalStateException("Problem to be fixed");
		...
		if (this.priorityMode == PriorityMode.NQPrioritized_ExpScoreAndProb) {
			for (int i = 0; i < this.nbClient; i++) {
				if (this.priorityQueue[i].size() > 0 && this.waitUntil[i] <= currentTime) {
					double lax = (Integer) this.priorityQueue[i].firstKey() - currentTime;
					...
					if (expectedScore > maxScore) { maxScore = expectedScore; scoreBasedOptIndex = i; }
				}
			}
		} else {
			for (int i = 0; i < this.nbClient; i++) {
				if (this.priorityQueue[i].size() > 0 && (Integer) this.priorityQueue[i].firstKey() < TWness
						&& this.waitUntil[i] <= currentTime && currentTime - this.waitUntil[i] < threshold) {
		...
		if (this.priorityMode != PriorityMode.NQPrioritized) {
			...
			if (this.priorityMode == PriorityMode.NQPrioritized_variant1) {
                ...
                considerWait = EDIndex != -1 && feasibleEDIndex != -1
                        && this.waitUntil[EDIndex] > this.waitUntil[feasibleEDIndex] && feasibleEDIndex != EDIndex;
				/*
				...
				if (considerWait) {
					...
					double lostPackets = lamda * (this.waitUntil[EDIndex] - this.waitUntil[feasibleEDIndex])
							/ this.packetTransTime;

					if (lostPackets < p) waitForED = true;
				}
			}
			// variant 3
			else if (this.priorityMode == PriorityMode.NQPrioritized_variant3) {
				double expStartTime = ED - expServiceTime;
				waitForED = EDIndex != -1 && feasibleEDIndex != -1 && feasibleEDIndex != EDIndex
						&& ED > currentTime + packetTransTime && this.waitUntil[EDIndex] > currentTime
				...
				if (waitForED) {
					 ...
					 * 
					 * double successProbGain = cumulativeExpDist(mu, slackTimeLong) - cumulativeExpDist(mu,
					 * slackTimeShort);
					 * 
		...
		if (this.priorityMode == PriorityMode.NQPrioritized_ExpScoreAndProb
			...
			if (this.priorityMode == PriorityMode.NQPrioritized_variant4) {
				...
				if (expStartTime < currentTime && feasibleED > currentTime + this.packetTransTime)
