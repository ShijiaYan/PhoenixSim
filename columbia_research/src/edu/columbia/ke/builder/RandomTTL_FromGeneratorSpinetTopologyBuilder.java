...
public class RandomTTL_FromGeneratorSpinetTopologyBuilder extends
		FromGeneratorSpinetTopologyBuilder {

	private double TTLSigma;

	public RandomTTL_FromGeneratorSpinetTopologyBuilder(
			...
			@ParamName(name = "Normalized Load Sigma", default_ = "0") double loadSigma, 
			@ParamName(name = "Normalized TTL Sigma", default_ = "0") double TTLSigma,
			Rate refBW) {
		super(gen, variant, linkDistMod, trafToSelf, refBW);
		this.TTLSigma = TTLSigma;
	}

	protected double getNewTTLMean(double oldTTLMean) {
		return oldTTLMean * (1 + TTLSigma * new Random().nextGaussian());
	}
