...
public class LayoutWorseCaseProperties {
    ...
    public double layoutBER;

    public LayoutWorseCaseProperties(int wavelengths) {
        this.wavelengths = wavelengths;
        this.ppList = new ArrayList<>(1);
    }

    public LayoutWorseCaseProperties(LayoutWorseCaseProperties... multiProperties) {
        if (multiProperties.length < 2) {
            throw new IllegalStateException("Should aggregate at least 2 layouts");
        } else {
            this.ppList = new ArrayList<>(1);
            ...
            for (LayoutWorseCaseProperties lp : multiProperties) {
                if (lp != null) {
                    ppList.addAll(lp.ppList);
                    propagationLatencyNS += lp.propagationLatencyNS;
                }
            }
    ...
    public ArrayList<PowerPenalty> getPowerPenalties() {
        return new ArrayList<>(this.ppList);
    }

    public double getLayoutBER() {
        return this.layoutBER;
    }
}
