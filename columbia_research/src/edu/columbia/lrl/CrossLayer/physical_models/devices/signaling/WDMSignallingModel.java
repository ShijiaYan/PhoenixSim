...
import edu.columbia.lrl.CrossLayer.physical_models.devices.modulations.ook_nrz.modarrays.CarrierDepletionRing_OOK_NRZ_ModulatorArrayModel;
...
public class WDMSignallingModel extends AbstractSignallingModel {
	...
	public WDMSignallingModel(
			@ParamName(name = "WDM Mod & Demod Model", defaultClass_ = CarrierDepletionRing_OOK_NRZ_ModulatorArrayModel.class) AbstractWDMModDemodModel wdmModel,
			@ParamName(name = "Demux", defaultClass_ = RingBasedFilterArrayModelAdaptiveIL.class) AbstractDemux demux) {
	...
	public Map<String, String> getAllParameters() {
		...
		m.put("WDM model", wdmModel.getClass().getSimpleName());
	...
	public ArrayList<PowerPenalty> getPowerPenalties(Execution ex, PhysicalParameterAndModelsSet modelSet,
			AbstractLinkFormat linkFormat, int i, int j) {
		ArrayList<PowerPenalty> all = new ArrayList<>();
		if (j > 1) {
			if (!demux.hasThroughCapability()) {
				// in the future, account for a 1x2 switch penalty
				throw new WrongExperimentException(
						"Cannot use multi demux (" + j + ") with demux of type " + demux.getClass().getSimpleName());
			}
		...
		if (i > 1) {
			if (wdmModel.modBankhasThroughCapability()) {
				throw new WrongExperimentException(
						"Cannot use multi mod (" + i + ") with mod of type " + wdmModel.getClass().getSimpleName());
			}
	...
	@Override
	public List<PowerConsumption> getPowerConsumptions(double opticalPowerAtReceiverdBm,
		...
		pc.addAll(
				wdmModel.getDemodulationAndDetectionPowerConsumption(opticalPowerAtReceiverdBm, modelSet, linkFormat));
		return pc;
	}
