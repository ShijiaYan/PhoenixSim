...
public class RingResonatorModel {
    ...
    public double getRoundTripLossdB() {
        ...
        double LossdB = Math.PI * 2 * radiusCm * this.getAlphaLossdBperCm();
        return LossdB;
    }
    ...
    public double getFSRnm() {
        return lambdaNm * lambdaNm / (Math.PI * 2 * this.radiusNm * this.nGroup);
    }

    public double get3dbBWhz() {
        double alphaM = this.getAlphaLossdBperCm() * 23.0D;
        double A = Math.PI * 2 * this.radiusMicron * 1.0E-6D * alphaM;
    ...
    public double get3dbBWnm() {
        ...
        double c = 3.0E8D;
        return this.lambdaNm * this.lambdaNm * 1.0E-9D / c * FWHMhz;
    }

    public double getQualityFactor() {
        return this.lambdaNm / this.get3dbBWnm();
    }

    public double getFinesse() {
        return this.getFSRnm() / this.get3dbBWnm();
    }
    ...
    public double getThroughTransmissionPeriodic(double detuningNm) {
        double alphaM = this.getAlphaLossdBperCm() * 23.0D;
        double Lsquared = Math.exp(-Math.PI * 2 * this.radiusMicron * 1.0E-6D * alphaM);
        double L = Math.sqrt(Lsquared);
        double tIn = this.getInputTcoeff();
        double tOut = this.getOutputTcoeff();
        double A = L * tIn * tOut;
        double B = L * tOut;
        double m = (double) Math.round(Math.PI * 2 * this.radiusNm * this.nGroup / this.lambdaNm);
        double phi = Math.PI * 2 * m * (detuningNm / this.lambdaNm);
    ...
    public static class ExponentialIdeal extends RingResonatorModel.AbstractBendingLossModel {
        ...
        public Map<String, String> getAllParameters() {
            return SimpleMap.getMap("K", this.K);
        }
    }
    ...
    public static class NoExtraBendingLossModel extends RingResonatorModel.AbstractBendingLossModel {
        ...
        public Map<String, String> getAllParameters() {
            return SimpleMap.getMap("Alpha prop in ring", this.alphaProp);
        }
    }
    ...
    public static class PowerlawBendingLossModel extends RingResonatorModel.AbstractBendingLossModel {
        ...
        public Map<String, String> getAllParameters() {
            return SimpleMap.getMap("a", this.a, "b", this.b, "c", this.c);
        }
    }
