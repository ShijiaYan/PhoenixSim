...
public class RingBasedFilterArrayModelAdaptiveIL extends AbstractRingBasedFilterArrayModel {
    ...
    public RingBasedFilterArrayModelAdaptiveIL(
            @ParamName(name = "Track Optimization Details", default_ = "false") boolean trackDetails,
            @ParamName(name = "Min Gap (nm)", default_ = "100") double minGap,
            @ParamName(name = "Max Gap (nm)", default_ = "400") double maxGap,
            @ParamName(name = "Optimization Steps", default_ = "1") int steps,
            @ParamName(name = "Truncation model to use", defaultClass_ =
                    ResidualsBased_SincSquare_Lorentzian_Model.class) Abstract_SincSquare_Lorentzian_TruncationModel truncModel,
    ...
    public ArrayList<PowerPenalty> getPowerPenalties(PhysicalParameterAndModelsSet modelSet,
        ...
        double wavelengths = linkFormat.getNumberOfChannels();
        ...
        double radiusMicron = 1000000.0D * lambda0 * lambda0
                / (2 * Math.PI * RingResonatorModel.getNeff() * fullFSR);
        ...
        for (bestIndex = 0; bestIndex < this.steps; bestIndex++) {
            ...
            if (Double.isInfinite(xtalk_pp[bestIndex]) || Double.isNaN(xtalk_pp[bestIndex])) {
                ...
                System.out.println("Modulation ER: " + modulationER);
                System.out.println("xtalk power[" + bestIndex + "] is " + truncFinal + ", xtalk_pp [" + bestIndex
                        + "] is " + xtalk_pp[bestIndex]);
                throw new WrongExperimentException("Crosstalk power penalty is infinite.");
            }
    ...
    public int findBestIndexAndStoreDataLocal(AbstractLinkFormat linkFormat, double[] insertionLoss, double[] gapVec,
                                              double[] xtalk_pp, double[] trunc_pp, double[] throughInsertionLoss,
                                              double[] demuxq, double ringRadius,
                                              Execution ex) {
    ...
    private double getXtalkPenalty(double filterQ, double v0, double deltaV, double rate) {
        ...
        double theta = Math.PI * 2 * nu * beta;
        double realPart = 1.0D - beta * beta;
        realPart += -1.0D * Math.exp(-Math.PI * 2 * nu) * Math.cos(theta) * (1.0D - beta * beta);
        realPart += Math.exp(-Math.PI * 2 * nu) * Math.sin(theta) * 2.0D * beta;
        realPart /= Math.pow(1.0D + beta * beta, 2.0D);
        double xtalk = 1.0D / (1.0D + beta * beta) - 1.0D / (Math.PI * 2 * nu) * realPart;
        return xtalk;
    }
    ...
    public static void main(String[] args) {
        ...
        for (double channelSpacing = 3.2E-10D; channelSpacing <= 3.2E-9D; channelSpacing += 3.2E-10D) {
            ...
            xtalk_pp = xtalkPPModel.getXtalkPP_DB(add_xtalk1, 9, 13);
