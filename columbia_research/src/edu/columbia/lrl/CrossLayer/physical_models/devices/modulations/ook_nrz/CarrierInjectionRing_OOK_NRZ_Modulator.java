...
public class CarrierInjectionRing_OOK_NRZ_Modulator extends Abstract_OOK_NRZ_Modulator {
	...
	private double Neff;
	private AbstractRingPNJunctionDriverPowerModel powerModel;

	public CarrierInjectionRing_OOK_NRZ_Modulator(OOK_NRZ_Receiver ookReceiver,
			@ParamName(name = "Modulator ring model", defaultClass_ = CarrierInjectionRing_OOK_NRZ_Modulator.DefaultModulatorRing.class) AbstractRingModel absRing,
	...
	private double getResShift(Constants ct, AbstractLinkFormat linkFormat) {
		int wavelengths = linkFormat.getNumberOfChannels();
		double channelSpacing = ct.getFullFSR() / (double) wavelengths * 1.0E9D;
	...
	public PowerPenalty getInsertionLossPowerPenalties(PhysicalParameterAndModelsSet modelSet,
			AbstractLinkFormat linkFormat, boolean isActive) {
		Constants ct = modelSet.getConstants();
		int wavelengths = linkFormat.getNumberOfChannels();
		double channelSpacing = ct.getFullFSR() / (double) wavelengths * 1.0E9D;
		double resLambda = ct.getCenterWavelength() * 1.0E9D;
		...
		double insertionLoss = -10.0D * Math.log10(transmission) * 0.0D;
	...
	public ArrayList<PowerConsumption> getPowerConsumption(PhysicalParameterAndModelsSet modelSet,
		...
		double modCons = this.powerModel.getAverageConsumption(
	...
	public static class DefaultModulatorRing extends QandERRingModel {

		public DefaultModulatorRing(@ParamName(name = "Modulator Q", default_ = "12000") double q,
