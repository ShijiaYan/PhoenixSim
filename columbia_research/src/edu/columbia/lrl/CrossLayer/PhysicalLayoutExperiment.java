...
public class PhysicalLayoutExperiment implements Experiment {
	...
	private boolean withPPdetails;
	private double utilization;
//	private boolean withPhyParams;

	public PhysicalLayoutExperiment(
			PhysicalLayout layout,
			AbstractLinkFormat linkFormat,
			PhysicalParameterAndModelsSet modeSet,
			@ParamName(name="Include power penalty details", default_="false") boolean withPPdetails) {
		this.layout = layout;
		this.modeSet = modeSet;
		this.linkFormat = linkFormat;
		this.withPPdetails = withPPdetails;
		this.utilization = 1;
	}
	...
	public void run(AbstractResultsManager man, AbstractResultsDisplayer dis) {
		...
		try {
			...
			dp.addResultProperty("available budget per channel dB", layout.getPowerBudgetPerChannel_dB(modeSet, linkFormat));
			if (!optimizedWavelengths) {
				double requiredBudget = layProp.getTotalPowerPenalty() + 10*Math.log10(wavelengths);

				// test
				dp.addResultProperty("laser eff",
						modeSet.getLaserModel().getLaserEfficiency(wavelengths,
				...
				dp.addResultProperty("Required laser power (mW)", PowerConsumption.dBmTomW(sensibility + requiredBudget));
			...
			general.addResultProperty("Energy-per-bit (on-chip) (pJ)", maxPowerWithoutLaser/(double)linkFormat.getAggregateRateInGbs());
			general.addResultProperty("Energy-per-bit (pJ)", maxPower/(double)linkFormat.getAggregateRateInGbs());
